...
def marcar_post_hecho_csv(num:int) -> None:
    ...
    tmp_file = "tmp.csv"
    with open(nuestro_csv, "r") as file, open(tmp_file, "w", newline='') as outFile:
    ...
    shutil.copy(tmp_file, nuestro_csv)
    remove(tmp_file)
...
def hacer_nuevo_post() -> None:
    ...
    if crear_imagen(nueva_frase, imagen):
        ...
        print("Copiando imagen final {}".format(nombre_nuevo))
        shutil.copy(nombre_nuevo, DIR_POSTS + "/" + imagen)
